...
> 适用版本：v3.1.0+（tzt-api 已迁出为独立仓库 [GXQuotes](https://github.com/seuzxh/GXQuotes)，本仓为 kline-qlib + 兼容壳）。本文档描述项目的整体架构、模块划分与数据流。
> 接口签名与参数明细见 [api-reference.md](api-reference.md)，实现层面的设计决策见 [design.md](design.md)。

## 1. 项目定位

kline-fetcher 仓库是 **A 股行情数据获取与 Qlib 格式转换工具集**（行情客户端 tzt-api 已迁至独立仓库 [GXQuotes](https://github.com/seuzxh/GXQuotes)，本仓保留 kline-qlib + 兼容壳），定位为量化回测框架（Qlib）的数据管道：

```
中焯行情 API（第三方数据源）
        │
        ▼
  GXQuotes 仓库（外部依赖，pip 安装 tzt-api）
...
https://github.com/seuzxh/GXQuotes（本机 ~/quant_projects/GXQuotes）
```
...
3. **包内默认配置**：GXQuotes 仓库 `tzt_api/config/kline_config.yaml`（随 tzt-api 包安装）。
...
| [GXQuotes docs/concept_plate_api.md](https://github.com/seuzxh/GXQuotes/blob/master/docs/concept_plate_api.md) | 概念板块接口深度文档（实测，随 tzt-api 迁至 GXQuotes） |
