...
class KLineFetcher:
    ...
    def fetch_day_kline_with_factor(self, code: str, count: Optional[int] = None, market: Optional[int] = None, begindate: Optional[str] = None, enddate: Optional[str] = None) -> Optional[List[Dict]]:
        ...
        for item in hfq_data:
            ...
            hfq_close = item.get("close")
            hfq_open = item.get("open")
            hfq_high = item.get("high")
            hfq_low = item.get("low")
            none_close = none_item.get("close") if none_item else None

            hfq_prices_valid = all(
                p is not None and p > 0
                for p in (hfq_open, hfq_high, hfq_low, hfq_close)
            )

            factor = None
            if (
                none_item
                and none_close not in (0, None)
                and hfq_prices_valid
            ):
                factor = float(hfq_close) / float(none_close)
            ...
            self.logger.warning(
                f"Invalid hfq data for code={code} date={item['date']}, "
                f"marking record as NaN (data source bug): "
                f"hfq O={hfq_open} H={hfq_high} L={hfq_low} C={hfq_close}, "
                f"none_close={none_close}, factor={factor}"
            )
            item["open"] = float("nan")
            item["high"] = float("nan")
            item["low"] = float("nan")
